export function ChessStats01() {
    return (
        ...
        After I made a plan for the database structure, ensuring it worked as a relational database, in order to avoid data inconsistency and data redundancy. I then wrote mongoose schemas to create the db models. <br /><br />             
        Next, I created Routes and Controllers for performing CRUD operations, such as adding new records or viewing current ones. So when a request is made by the user, the endpoint(route) sent to the server runs a certain task assigned by the controller.<br />&nbsp;
